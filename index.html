...
if (savedData) {
  try {
    ...
    energy = savedData.energy !== undefined ? savedData.energy : 100;
    tapPower = savedData.tapPower || 1;
    level = savedData.level || 1;
    ownedSkins = savedData.ownedSkins || ['gold'];
    activeSkin = savedData.activeSkin || 'gold';
    lastBonusDate = savedData.lastBonusDate || null;
    bonusStreak = savedData.bonusStreak || 0;
    autoLevel = savedData.autoLevel || 0;
    invested = savedData.invested || 0;
    lastOnlineTime = savedData.lastOnlineTime || Date.now().toString();
    offlineEarnings = savedData.offlineEarnings || 0;
    tapCount = savedData.tapCount || 0;
    rouletteWins = savedData.rouletteWins || 0;
    completedAchievements = savedData.completedAchievements || [];
...
function resetGame() {
  ...
  autoLevel = 0;
  invested = 0;
  lastOnlineTime = Date.now().toString();
  offlineEarnings = 0;
  tapCount = 0;
  rouletteWins = 0;
